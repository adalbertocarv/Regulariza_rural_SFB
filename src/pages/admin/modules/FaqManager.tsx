...
export default function FaqManager() {
  ...
  const openCreate = () => {
    setEditing(null);
    setForm({ ...emptyForm, orderNum: (items.length + 1) });
    setShowModal(true);
  };
  const openEdit = (item: Faq) => { setEditing(item); setForm({ ...item }); setShowModal(true); };
        ...
        <Modal title={editing ? 'Editar Pergunta' : 'Nova Pergunta'} onClose={closeModal}>
          <form onSubmit={handleSave} className="space-y-4">
            ...
            <Field label="Resposta">
              <textarea className={textareaClass} rows={5} value={form.answer || ''} onChange={(e) => setForm((f) => ({ ...f, answer: e.target.value }))} />
            </Field>
            <Field label="Ordem">
              <input className={inputClass} type="number" min={1} value={form.orderNum || 1} onChange={(e) => setForm((f) => ({ ...f, orderNum: Number(e.target.value) }))} />
            </Field>
